...
async def test_raw_query_readonly_blocks_writes(tmp_path):
    ...
    rows = await storage.raw_query("SELECT subdomain FROM subx_subdomain WHERE target = 'example.com';")
    assert len(rows) == 1
    assert rows[0]["subdomain"] == "test.example.com"
    
    with pytest.raises(OperationalError) as exc_info:
        await storage.raw_query(
            "INSERT INTO subx_subdomain (target, subdomain, source_plugin) VALUES ('example.com', 'evil.example.com', 'Evil');"
        )
