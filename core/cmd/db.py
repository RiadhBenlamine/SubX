...
def register(app: typer.Typer) -> None:
    """Register the db command on the given Typer app."""
    app.command("db", context_settings=HELP_NAMES)(db)
...
async def _db(
    ...
    if raw_query:
        await _db_raw_query(service, raw_query, output_n, output_x)
        return

    if not domain:
        if any([delete, filter_plugin, new_since, output_n, output_x, web]):
            error("Filters and output flags require -d <domain>.")
        await _db_summary(service)
        return

    if delete:
        if output_n or output_x:
            warn("-oN / -oX are ignored when using --delete.")
        await _db_delete(service, domain)
        return

    await _db_query(service, domain, web, filter_plugin, new_since, output_n, output_x)


async def _db_summary(service: DbService) -> None:
...
async def _db_delete(service: DbService, domain: str) -> None:
    count = await service.delete_domain(domain)
    if count == 0:
        console.print(f"[dim]  No records found for[/dim] [bold white]{domain}[/bold white]\n")
    else:
        from core.ui.console import success
        success(f"Deleted [bold white]{count}[/bold white] records for [bold white]{domain}[/bold white]")
...
async def _db_query(
    ...
    since_dt = None
    if filter_plugin:
        info(f"Target : [bold white]{domain}[/bold white]  Plugin : [bold white]{filter_plugin}[/bold white]")
    elif new_since:
        try:
            ...
            return
        info(f"Target : [bold white]{domain}[/bold white]  New since : [bold white]{new_since}[/bold white]")
    else:
        info(f"Target : [bold white]{domain}[/bold white]")

    rows = await service.get_subdomains(domain, filter_plugin=filter_plugin, new_since=since_dt)
    ...
    if web:
        render_db_rows_web(rows, domain)
    else:
        render_db_rows(rows, domain)
    ...
    if output_n:
        write_output(subdomains, output_n, separator="\n")

    if output_x:
        sep, file = parse_ox(output_x)
        write_output(subdomains, file, separator=sep)
...
async def _db_raw_query(
    ...
    if output_n:
        write_output(values, output_n, separator="\n")

    if output_x:
        sep, file = parse_ox(output_x)
        write_output(values, file, separator=sep)
